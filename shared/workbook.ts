...
export async function parseWorkbook(file:File):Promise<Source>{
 if(file.size>10*1024*1024)throw Error('ไฟล์ต้องไม่เกิน 10 MB');const Excel=await import('exceljs');const wb=new Excel.Workbook();await wb.xlsx.load(await file.arrayBuffer());
